...
- Github API는 API 요청 횟수 제한 적용되기 때문에 조심하셔야 하는데 1분 당 10회의 요청 제한 정책을 가지고 있고, 이 한도를 넘기게 되면 1분 기다리고 다시 하시면 됩니다.
...
여기서 정의한 시간을 넘기면 데이터가 오래된 것으로 간주되어 백그라운드로 데이터를 다시 불러오게 됩니다. default 값으로는 0으로 쿼리가 성공 직수 데이터가 최신이 아닌 상태로 정의되어 리포커싱할 때마다 계속 새로고침이 일어나게 됩니다.
...
const { isLoading, isError, data } = useQuery({
  queryKey: ['repositories'],
  queryFn: getRepositories,
  staleTime: 60 * 1000, // 밀리초 단위이기 때문에 -> 1분에 해당됩니다.
})
```

github API를 통해서 GET을 처리하는 방법을 배웟습니다. 나머지 CRUD 부분은 cardatavase를 하면서 useQuery를 사용할 예정입니다.
useMutauion() 훅이 좀 중요합니다.

# 유용한 리액트용 서드파티 컴포넌트

## 서드파티 리액트 컴포넌트 설치 방식
필요한 컴포넌트를 찾을 수 있어야하는데,
chrome -> google에서 awesome-react-components을 검색하면 github 페이지 하나 나오는데 거기서 필요한 컴포넌트(예를 들어 table / chart / button etc...)
...
- npm install을 하게 되면 package.json 파일에 컴포넌트에 의존성을 추가하게 됩니다.
  - Java에서는 build.gradle에 저희가 직접 추가하는 방식이였습니다.
